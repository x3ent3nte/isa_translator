...
fun generateArmConstraints () = 
	...
	DefineSort "Sequence" (ArraySort (BitVecSort "3") "State") ^ "\r\n" ^
		...
		(ForAll
			...
			(Exists
				[
					...
					("imm_used", "Bool"),
					("barrel_op", "BarrelOp"),
					("barrel_num", BitVecSort "32")
				]
				(Let
					[
						("pre", Select "sequence" "x"),
						("post", Select "sequence" (BitVecAdd "x" "#b001") )
					]
					(Or [
						(Equals "x" "#b111"),
						(And[
							(Equals (BitVecAdd (Select "pre" "PC") "#b00000000000000000000000000000100") (Select "post" "PC")), "\r\n",
							(Or[
								...
								(Let 
									[
										("flex_val", 
											(Let [("val_to_shift",
													(If "imm_used"
														(BitVecRotateRight (BitVecConcat "#b000000000000000000000000" (Extract "7" "0" "imm")) "0")
														(Select "pre" "ro")
													)
													)]
											...
											(And [
												...
												(Or [
													...
													(And [
														(Equals "flag" S),
														(Or [
															(Not (Equals (Select "post" "rd") "#b00000000000000000000000000000000")),
															(Equals (Extract "30" "30" (Select "post" CPSR)) "#b1")
														]),
														(Or [
															(Equals (Select "post" "rd") "#b00000000000000000000000000000000"),
															(Equals (Extract "30" "30" (Select "post" CPSR)) "#b0")
														])
													])
												])
...
(* fun main () = print (Int.toString (generateArmConstraints())) *)
...
fun main () = writeFile "sml_gen.smt2" (generateArmConstraints ())

(*
TODO

function that produces bitvec value
expr/grammar creation

have array storing the instructions
